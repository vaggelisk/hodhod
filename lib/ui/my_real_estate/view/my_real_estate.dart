...
class MyRealEstate extends StatelessWidget {
  MyRealEstate({super.key});

  MyRealEstateController controller = Get.put(MyRealEstateController());
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar(
          isShowBack: false,
          centerTitle: true,
          title: Text(LangKeys.myRealEstate.tr,
              style: AppTextStyles.semiBoldTextStyle.copyWith(
                  color: Colors.black, fontSize: 16.sp, height: 1.h))),
      body: GetBuilder<MyRealEstateController>(
          id: 'updateList',
          builder: (controller) {
            return RefreshIndicator(
                ...
                child: PagedListView<int, RealEstates>(
                  ...
                  builderDelegate: PagedChildBuilderDelegate<RealEstates>(
                      itemBuilder: (context, item, index) => ItemMyRealEstate(
                            data: item,
                            onTap: () {
                            ...
                            onDelete: () {
                              confirmDeleteBottomSheet(LangKeys.delete.tr,
                                  LangKeys.deleteRealEstatesMsg.tr, () {
                                Get.back(closeOverlays: false);
                                controller.deleteRealEstates(item.id ?? 0);
                              });
                            },
                            onEdit: () {
